...
app.use((req, res, next) => {
    console.log('Request Method:', req.method, 'OriginalUrl:', req.originalUrl);
...
app.get('/topstories', (req, res, next) => {
    ...
    (error, response, body) => {
        if (error || response.statusCode !== 200 ) {
            return next(new Error('Requesting top stories'));
        }
            ...
            topStories.slice(0, limit).map(story => {
                return new Promise ((resolve, reject) => {
                    request(
                        ...
                        (error, response, body) => {
                            if (error || response.statusCode !== 200){
                                return reject(new Error('Error occured in top stories items'));
                            }
